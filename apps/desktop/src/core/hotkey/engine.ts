...
/**
 * Pure state machine that turns raw key down/up events into dictation actions.
 *
 * Semantics (all simultaneously available on the same push-to-talk chord):
 *  - Press chord            -> `start` immediately (audio must begin on press, not release).
 *  - Hold >= tapThreshold, release -> `stop` (transcribe + insert).
 *  - Hold <  tapThreshold, release -> a *tap*:
 *        trigger 'tap'        -> `lock` (hands-free; keep recording)
 *        trigger 'double-tap' -> `stop`; a second press within doubleTapWindow starts locked
 *        trigger 'off'        -> `stop`
 ...
 *
 * The engine never owns timers; the host feeds monotonic timestamps.
 */
export class HotkeyEngine {
  ...
  constructor(private config: HotkeyEngineConfig) {
  ...
  applyConfig(config: HotkeyEngineConfig): void {
    ...
    this.chords = {
      ptt: c(config.pushToTalk),
      handsFree: c(config.handsFree),
      command: c(config.commandMode)
    }
  }

  /** Forget all pressed keys (call when the hook restarts or the OS lost track). */
  reset(): void {
    this.down.clear()
    this.session = null
    this.lastTapAt = null
  ...
  get isLocked(): boolean {
    return this.session?.locked ?? false
  }

  get currentMode(): DictationMode | null {
    return this.session?.mode ?? null
  }

  /** Host calls this when a session ended for reasons the engine did not initiate (e.g. max duration, UI stop). */
  externalStop(): void {
    this.session = null
  }

  /** Host calls this when the UI starts a hands-free session (e.g. tray click). */
  externalStart(mode: DictationMode, now: number): void {
    this.session = { mode, chord: 'handsFree', startedAt: now, locked: true }
  }

  keyDown(rawCode: number, now: number): HotkeyAction[] {
    const code = canonicalKey(rawCode, this.config.sideSensitive)
    if (this.down.has(code)) return [] // OS key-repeat
    this.down.add(code)

    if (code === Key.Escape) {
      if (this.session && this.config.escapeCancels) {
        this.session = null
        return [{ type: 'cancel' }]
      }
      return []
    ...
    if (this.session) {
      ...
      if (chord === 'handsFree' && this.session.mode !== 'command') {
        this.session.locked = true
        this.session.chord = 'handsFree'
        return [{ type: 'lock' }]
      }
      // A different chord completing during a hold (e.g. added a key) is ignored.
      return []
    }

    if (chord === 'handsFree') {
      this.session = { mode: 'hands-free', chord, startedAt: now, locked: true }
      return [{ type: 'start', mode: 'hands-free' }]
    }

    if (chord === 'command') {
      this.session = { mode: 'command', chord, startedAt: now, locked: false }
      return [{ type: 'start', mode: 'command' }]
    }

    // push-to-talk
    const isDoubleTap =
      this.config.handsFreeTrigger === 'double-tap' &&
      this.lastTapAt !== null &&
      now - this.lastTapAt <= this.config.doubleTapWindowMs
    this.lastTapAt = null
    if (isDoubleTap) {
      this.session = { mode: 'hands-free', chord, startedAt: now, locked: true }
      return [{ type: 'start', mode: 'hands-free' }]
  ...
  private newlyActiveChords(code: number): ChordId[] {
    ...
    return ids
      .filter((id) => {
        const keys = this.chords[id]
        return keys.length > 0 && keys.includes(code) && keys.every((k) => this.down.has(k))
      })
      .sort((a, b) => this.chords[b].length - this.chords[a].length)
  }
