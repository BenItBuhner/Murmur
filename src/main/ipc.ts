...
export function registerIpc(deps: IpcDeps): void {
  ...
  ipcMain.handle(IPC.sttTest, async (_e, override?: { kind?: Settings['stt']['kind']; baseUrl?: string; apiKey?: string; model?: string }): Promise<ProviderTestResult> => {
    ...
    } catch (err) { const e = err as { suggestedModels?: string[] }; return { ok: false, message: friendlyError(err), suggestedModels: e.suggestedModels ?? [] } }
  })
  ...
  ipcMain.handle(IPC.injectTest, async (_e, text: string) => { const s = settings.get(); await new Promise((r) => setTimeout(r, 1200)); return injectText(text, { method: s.injection.method, restoreClipboard: s.injection.restoreClipboard, restoreClipboardDelayMs: s.injection.restoreClipboardDelayMs, typeChunkSize: s.injection.typeChunkSize, typeChunkDelayMs: s.injection.typeChunkDelayMs, waitForKeysUp: () => hook.waitForKeysUp(1000) }) })
  ipcMain.handle(IPC.pipelinePreview, (_e, raw: string) => { const s = settings.get(); return runPipeline(raw, controller.pipelineOptions(s)) })
