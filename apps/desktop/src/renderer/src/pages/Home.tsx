...
export function LatencyBar({ t }: { t: HistoryEntry['timings'] }): React.JSX.Element {
  // One hue, five tints: the bar reads as proportions, not as a rainbow.
  const parts = [
    { label: 'Silence trim', ms: t.vadMs, color: 'bg-foreground/20' },
    { label: 'Speech to text', ms: t.sttMs, color: 'bg-foreground' },
    { label: 'Cleanup', ms: t.formatMs, color: 'bg-foreground/55' },
    { label: 'Smart format', ms: t.llmMs, color: 'bg-record' },
    { label: 'Insert', ms: t.injectMs, color: 'bg-foreground/35' }
  ].filter((p) => p.ms > 0)
